...
class UserManager:
    def __init__(self):
        try:
            self.customers = json.load(open("customers_list.json"))
        except Exception:
            self.customers = []
    ...
    def binary_search(self, key: str, index):
        results = []
        ...
        if index == 4:
            ...
            while low <= high:
                mid = (low + high) // 2
                value = self.customers[mid][index].lower()

                if key in value:
                    results.append(self.customers[mid])
                    left, right = mid - 1, mid + 1
                    while left >= 0 and key.lower() in self.customers[left][index].lower():
                        results.append(self.customers[left])
                        left -= 1
                    while right < len(self.customers) and key.lower() in self.customers[right][index].lower():
                        results.append(self.customers[right])
                        right += 1
                    break
                elif value < key.lower():
                    low = mid + 1
                else:
                    high = mid - 1
        print(results)
        ...
        if results:
            for user in results:
                self.search_results.insert(tk.END,
                                           f"Name: {user[0]}, Location: {user[1]}, Address: {user[2]}, Contact: {user[3]}, Interests: {user[4]}")
        else:
